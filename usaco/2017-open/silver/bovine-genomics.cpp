...
// N Cows with spots, N cows without spots
// Genome = Length M, [A,C,G,T]
...
int main() {
    ...
    int N, M;
    cin >> N >> M;
    vector<vector<int>> spot(N, vector<int>(M, 0)), plain(N, vector<int>(M, 0));
    string row;

    // Input Spot and Plain cows
    for (int i = 0; i < N; i++) {
        cin >> row;
        for (int j = 0; j < M; j++) {
            if (row[j] == 'T') {
    ...
    for (int i = 0; i < M; i++) {
        for (int j = i + 1; j < M; j++) {
            for (int k = j + 1; k < M; k++) {
                array<bool, 64> lookup = {false};
                for (vector<int> &cow : spot) {
                    lookup[(cow[i] << 4) + (cow[j] << 2) + cow[k]] = true;
                }
                bool confirmed = true;
                for (vector<int> &cow : plain) {
                    if (lookup[(cow[i] << 4) + (cow[j] << 2) + cow[k]])
                        confirmed = false;
                        break;
                }
                if (confirmed)
